...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // ZettelKasten: starting difficulty is 1 / 2^20
...
const string strMessageMagic = "ZettelKasten Signed Message:\n";
...
{
    const char* pszTimestamp = "Schlangen und Katzen koennen nicht Schlatzen zeugen";
    ...
    block.nVersion = 1;
    block.nTime    = 1522171630; //fTestNet? 1496033996 : 1495841000; //1406620001 : 1406620000;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey* preservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ZettelKastenMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ZettelKastenMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ZettelKastenMiner(CWallet *pwallet)
{
    printf("ZettelKastenMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ZettelKasten-miner");
    ...
    try { for (;;) {
       while (vNodes.empty() && !fTestNet)
        ...
        printf("Running ZettelKastenMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ZettelKastenMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ZettelKastenMiner, pwallet));
}
